...
//-----------------------------------------------
function xxxbuildAbmeldeListe(Abmeldungen) {
//-----------------------------------------------
    if (!Abmeldungen ) {
        return "";
    }
    ...
    // Durchlaufe die Namen und füge Zeilenumbrüche ein
    for (let i = 0; i < abwesendeNamen.length; i++) {
        finalOutput.push(abwesendeNamen[i]);
        
        // Füge einen Zeilenumbruch und eine neue Zeile hinzu, wenn:
        // 1. Es nicht der letzte Eintrag ist (i < length - 1)
        // 2. Wir am Ende einer Gruppe von NAMES_PER_LINE sind ((i + 1) % NAMES_PER_LINE === 0)
        if (i < abwesendeNamen.length - 1 && (i + 1) % NAMES_PER_LINE === 0) {
            finalOutput.push('\n'); // Fügt einen harten Zeilenumbruch ein
        } else if (i < abwesendeNamen.length - 1) {
            finalOutput.push(', '); // Fügt Komma und Leerzeichen ein
        }
    }

    // Verbinde das Array zu einem einzigen String
...
//-----------------------------------------------
const abmeldeListe = buildAbmeldeListe(namentlicheAbmeldungenCache[aktTerminIndex])
      console.log("createRapport, abmeldeListe:", abmeldeListe);
const anmeldungen = namentlicheAnmeldungenCache [aktTerminIndex];

apiCall('updateSheetRapport', {
    termin: aktTermin,
    anmeldungen: anmeldungen,
    abmeldungen: abmeldeListe
  })
  .then(response => {
    console.log("Rapport erfolgreich erstellt:", response);
  })
